...
export default function BookList({ pageTitle, category }) {
      ...
      Promise.resolve(apiClient.getUserProfile(username)).then((profile) => {
        Promise.resolve(apiClient.retrieveEntries()).then((results) => {
          let fetchedEntries = results.map((bookEntry, index) => {
            if (
              bookEntry.status === category ||
              pageTitle === "All Book Entries"
            ) {
              let isFavorite =
                profile.favorite_book === null
                  ? false
                  : bookEntry.book.id === profile.favorite_book.id;
              return (
                <BookCard
                  ...
                  progress={
                    (bookEntry.current_page / bookEntry.book.page_count) * 100
                  }
      ...
      <>
        ...
        >
          ...
          <AlertDialogContent>
            ...
            <AlertDialogCloseButton />
            <AlertDialogBody>
              Are you want to delete "{currentEntry.book.title}" from your
              books?
            </AlertDialogBody>
            <AlertDialogFooter>
      ...
      <>
        ...
        >
          ...
          <AlertDialogContent>
            ...
            <AlertDialogCloseButton />
            <AlertDialogBody>
              Are you want to make "{currentEntry.book.title}" your favorite
              book?
            </AlertDialogBody>
            <AlertDialogFooter>
              <Button onClick={onClose}>No</Button>
  ...
  const updateEntry = () => {
    Promise.resolve(apiClient.editEntry(currentEntry.id, currentEntry)).then(
      () => onClose()
    );
  };

  const deleteEntry = () => {
    Promise.resolve(apiClient.deleteEntry(currentEntry.id)).then(() =>
      alertDiagHandler.onClose()
    );
  };
  ...
  const selectFavorite = () => {
    Promise.resolve(apiClient.retrieveUsername()).then((username) => {
      Promise.resolve(
        apiClient.editProfile(username, {
          favorite_book_id: currentEntry.book.id,
        })
      ).then((profile) => {
  ...
  return (
    ...
    >
      ...
      <BookAlertDiag
        isOpen={alertDiagHandler.isOpen}
        onClose={alertDiagHandler.onClose}
      />
      <FavoriteAlertDiag
        isOpen={favoriteAlertDiagHandler.isOpen}
        onClose={favoriteAlertDiagHandler.onClose}
      />
      <EntryForm
        book={currentEntry.book}
        current={currentEntry.current_page}
        isOpen={isOpen}
        onOpen={onOpen}
        onClose={onClose}
        modifier={updateEntryState}
        submitEntry={updateEntry}
      />
      <SlideFade in={bookEntries.length > 0} out={bookEntries.length === 0}>
        <SimpleGrid
          spacing={5}
          alignItems={"center"}
